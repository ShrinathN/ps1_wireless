...
int main(void)
{
  ...
  NRF_Init();
  NRF_SetAddressRx((BYTE[]){0x12,0x23,0x34}, 3, 0);
  NRF_WriteRegister(0x11, 3);
  ...
  while (1)
  {
    ...
    a = NRF_ReadRegister(REG_STATUS);
    NRF_ReadRxFIFO(buffer, 3);
    NRF_WriteRegister(REG_STATUS, 0b01111111 & a);
    buffer[0] ^= 0xff;
    buffer[1] ^= 0xff;

    USBD_HID_SendReport(&hUsbDeviceFS, buffer, 2);
    LED_TOGGLE;
    /* USER CODE END WHILE */
